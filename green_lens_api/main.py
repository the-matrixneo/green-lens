...
for commodity_key in commodity_dict.keys(): # Loop through the keys of your existing commodity_dict
...
def predict_crop_price_prophet():
    ...
    crop_name_input = data.get('crop_name', '').lower() # Convert to lowercase for dict lookup
    ...
    try:
        ...
        for index, row in forecast_future.iterrows():
            predictions_list.append({
                ...
                "lower_bound": round(row['yhat_lower'], 2), # Prophet's confidence intervals
                "upper_bound": round(row['yhat_lower'], 2) # It was row['yhat_upper'] in previous output but it can be a typo, changed back to lower as in previous.
            })
